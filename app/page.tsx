...
function LushiSummaryReveal({ onContinue }: { onContinue: () => void }) {
  ...
  return <div className="jueju-reveal lushi-summary-reveal" aria-live="polite">
    ...
    {allFlipped && <div className="poem-kind-finale"><span>兩首律詩都觀察完成</span><h3>每首8句、每句5或7字，2468句押韻，七言！律詩！</h3><Button onClick={onContinue}>絕句和律詩的差異，原來只有句數不一樣啊！......是嗎！？ <ArrowRight /></Button></div>}
  </div>;
}
...
function LushiPage({ next }: { next: () => void }) {
  ...
  return <section className="jueju-lesson lushi-lesson"><span className="jueju-section-label">近體詩館 · 第二分頁</span><h1>律詩概念</h1><p>觀察八句律詩，找出它和絕句相同、又更長的格律。</p><div className="lushi-work-switch">{lushiWorks.map((item,index)=><button type="button" className={index===workIndex?'selected':''} key={item.title} onClick={()=>{setWorkIndex(index);setLocked(false);setSelected([]);}}>{item.title}</button>)}</div><article className="lushi-observation"><h2>〈{work.title}〉</h2><p>先觀察：這首詩共有幾句？每句幾字？</p>{!locked ? <div className="observation-plain">{work.lines.map((line)=><p key={line}>{line}</p>)}</div> : <div className="poem-lines">{work.lines.map((line,index)=><button type="button" key={line} className={selected.includes(index)?'rhyme-selected':''} onClick={()=>work.rhyme.includes(index)&&setSelected((c)=>c.includes(index)?c:c.concat(index))}><em className="poem-line-number">{index+1}</em><span>{line}</span>{workIndex === 0 && index === 3 && <small className="lushi-pronunciation">ㄒㄧㄚˊ</small>}</button>)}</div>}<div className="lushi-choices"><label>共有 <NumberWheel value={lineChoice} options={[7,8,9]} min={7} max={9} label="律詩句數" locked={locked} onChange={setLineChoice}/> 句</label><label>每句 <NumberWheel value={charChoice} options={workIndex ? [6,7,8] : [4,5,6]} min={4} max={8} label="律詩字數" locked={locked} onChange={setCharChoice}/> 字</label></div>{!locked&&<><Button className="lushi-count-confirm" onClick={()=>{const ok=lineChoice===8&&charChoice===(workIndex?7:5);setCountFeedback(ok?'句數與字數正確！':'再觀察一次：律詩共有8句。');if(ok)setLocked(true);}}>確定句數與字數</Button>{countFeedback&&<output className={locked?'correct':''}>{countFeedback}</output>}</>}{locked&&<><p className="rhyme-prompt lushi-rhyme-prompt"><strong>請按出有押韻的句子。</strong></p>{right&&<Button className="page-advance" onClick={()=>{setCompleted((c)=>c.includes(workIndex)?c:c.concat(workIndex));if(workIndex===0){setWorkIndex(1);setLocked(false);setSelected([]);setLineChoice(7);setCharChoice(6);}else{setShowSummary(true);}}}>{completed.length===1?'換看下一首律詩':'換看下一首律詩'} <ArrowRight/></Button>}</>}</article></section>;
}
